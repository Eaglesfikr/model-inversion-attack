...
def one_hot_preds(preds):
    t = np.argmax(preds, axis=1)# 找到预测类别的索引
    r = np.zeros(preds.shape)   # 初始化独热编码数组
...
#将类标签转换为独热编码格式。
def one_hot_class(a):
    b = np.zeros((len(a), np.max(a).astype(int)+1), np.float32)# 初始化独热编码数组
    b[np.arange(len(a)), a.astype(int)] = 1                    # 设置独热编码
    return b

#
def unpack_facedataset(path = 'E:/model_inv/archive'
, sz=None):
...
#归一化图像，使其值在 [-1, 1] 之间，并使用全局对比度标准化来增强图像。并执行 GPU 矩阵运算来进一步处理图像。
...
#与 normalize 类似，但省略了对比度标准化和均值减去步骤。
